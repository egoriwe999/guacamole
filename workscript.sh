...
# https://apache.org/dyn/closer.lua/guacamole/1.4.0/binary/guacamole-auth-sso-1.4.0.tar.gz?action=download
...
menu_header
	...
	echo -n "${Green} Enter the CAS Authorization Endpoint : ${Yellow}"
	read CAS_AUTHORIZATION_ENDPOINT
	while [ -z "$CAS_AUTHORIZATION_ENDPOINT" ]
	do
		echo -n "${Green} Enter the CAS Authorization Endpoint : ${Yellow}"
		read CAS_AUTHORIZATION_ENDPOINT
	done
#	CAS_AUTHORIZATION_ENDPOINT=${CAS_AUTHORIZATION_ENDPOINT:-Default Endpoint}

	echo -n "${Green} Enter the redirect URI for CAS : ${Yellow}"
	read CAS_REDIRECT_URI
	while [ -z "$CAS_REDIRECT_URI" ]
	do
		echo -n "${Green} Enter the redirect URI for CAS : ${Yellow}"
		read CAS_REDIRECT_URI
	done
#	CAS_REDIRECT_URI=${CAS_REDIRECT_URI:-URI}

## Optional Properties
	echo -n "${Green} Enter the CAS Clearpass Key : ${Yellow}"
	read CAS_CLEARPASS_KEY
	echo -n "${Green} Enter the CAS Group Attribute : ${Yellow}"
	read CAS_GROUP_ATTRIBUTE
	echo -n "${Green} Enter the CAS Group Format : ${Yellow}"
	read CAS_GROUP_FORMAT
	echo -n "${Green} Enter the CAS LDAP Base DN : ${Yellow}"
	read CAS_GROUP_LDAP_BASE_DN
	echo -n "${Green} Enter the CAS LDAP Attribute : ${Yellow}"
	read CAS_GROUP_LDAP_ATTRIBUTE
}
...
case $PRIME_AUTH_TYPE in
	...
	"CAS")
		...
		[ "${CAS_CLEARPASS_KEY}" ] && echo "${Green} CAS Clearpass Key : ${Yellow}${CAS_CLEARPASS_KEY}"
		[ "${CAS_GROUP_ATTRIBUTE}" ] && echo "${Green} CAS Group Attribute : ${Yellow}${CAS_GROUP_ATTRIBUTE}"
		[ "${CAS_GROUP_FORMAT}" ] && echo "${Green} CAS Group Format : ${Yellow}${CAS_GROUP_FORMAT}"
		[ "${CAS_GROUP_LDAP_BASE_DN}" ] && echo "${Green} CAS LDAP Base DN : ${Yellow}${CAS_GROUP_LDAP_BASE_DN}"
		[ "${CAS_GROUP_LDAP_ATTRIBUTE}" ] && echo "${Green} CAS LDAP Attribute : ${Yellow}${CAS_GROUP_LDAP_ATTRIBUTE}"
		;;
esac
...
additionalinstall ()
{
  { yum install -y nano wget mlocate open-vm-tools yum-utils nano net-tools wget telnet mlocate policycoreutils-python\
    autoconf automake yum-utils firewalld; }&
s_echo "y" "${Bold}Installing additional packages...    "; spinner
...
if [ $GUAC_SOURCE == "Git" ]; then
   ...
   { wget "${GUAC_URL}binary/${GUAC_SSO}.tar.gz" -O ${GUAC_SSO}.tar.gz; } &
   s_echo "n" "-Downloading SSO bundle extension...    "; spinner

   # Decompress SSO bundle Extension
   {
      tar xzvf ${GUAC_SSO}.tar.gz
      rm -f ${GUAC_SSO}.tar.gz
      mv ${GUAC_SSO} extension
   } &
   s_echo "n" "-Decompressing Guacamole CAS Extension...    "; spinner

   # Copy CAS Extension to Extensions Directory
   { mv -v extension/${GUAC_SSO}/cas/${GUAC_CAS}.jar ${LIB_DIR}extensions/; } &
   s_echo "n" "-Moving Guacamole CAS extension to extensions dir...    "; spinner
fi
...

{
  ...
  ln -sf -T /var/lib/tomcat/webapps/guacamole /opt/guacamole
  ln -sf -T /var/lib/tomcat/webapps/cas /opt/cas
} &
